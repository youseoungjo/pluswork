 ...
 */
@Service
public class IN1101Service {
	/**
	 * log 처리를 위한 변수 선언
	 */
	private static final Logger logger = LoggerFactory.getLogger(IN1101Service.class);
	 ...
	 */
	public List<Map<String, Object>> retrieveIN1101(Map<String, Object> data) {
		return dao.queryForMapList("in1101.retrieveIN1101", data);
	}
	 ...
	 * @return List<Map<String, Object>>
	 */
	public void saveIN1101List(GridData<HashMap> gridData) {
		gridData.forEachRow(new AbstractRowStatusCallback<HashMap>() {
			...
			public void update(HashMap newRecord, HashMap oldRecord, int arg2) {
				...
				if ( "".equals(oldRecord.get("chkRsltNo")) ){
					...
					dao.update("in1101.saveIN1101List", newRecord);
				} else {
					newRecord.put("fnlEditUserId", UserInfo.getUserId());

					dao.update("in1101.updateIN1101List", newRecord);
				}
			}
